module Chulabooster
  module Mappers
    class StudentCourses < Base
      ...
      # 31,079 local / 49,502 CB rows): Grade#year is Gregorian/CE (confirmed real range 2018..2025),
      # NOT Buddhist Era like course.revision_year/admission_year_be — so CB's already-CE academic_year
      # must NOT be converted to BE here. And CB's semester_code is a string like "s1"/"s2"/"s3", not a
      # plain integer like local's Grade#semester — Convert.semester_number strips the "s" prefix.
      def local_key(g)
        [
