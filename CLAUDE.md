...
- **`students.study_track`** (`regular`/`special`, null = unknown/not applicable): the M.Sc.-CS
  ภาคนอกเวลาราชการ track split (the department's "CT" cohorts, intakes 2533–2560). Backfilled from
  registrar evidence via `students:backfill_study_track` (dry-run default, `COMMIT=1`); deliberately
  has NO importer attribute. The synthetic program `0999` was dissolved onto the regular CS lineage
  (`programs:dissolve_0999`) — do not re-add it; the real B.E. 2540 registration is CB `175211001997`
  (the special program itself). Importer upserts never overwrite an existing `program_id`
  (fill-blank-only, `Importers::Base#update_protected_fields`). Spec:
...
- `solid_queue` worker restart: `ssh 10.0.5.12 'sudo -n systemctl restart solid_queue'` — passwordless since 2026-07-31 via an exact-command NOPASSWD rule in `/etc/sudoers.d/solid-queue-restart`. Needed only when job-side code changes.
