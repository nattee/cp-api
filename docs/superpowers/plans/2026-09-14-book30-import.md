...
```scss
// Row provenance (students.source shares the vocabulary of grades.source): reuse the
// existing .badge-imported / .badge-manual / .badge-chulabooster classes — same concept,
// same colours on both pages — and add only the new value.
.badge-book30 { background-color: rgba(#7c6fdc, 0.2); color: #a89ff0; border: 1px solid rgba(#7c6fdc, 0.4); }
```
...
> **Terminology note (2026-09-15):** what this plan calls "placeholder" students are now called *students recorded from the book* in code, UI and docs; only the student ID is synthetic. Identifiers renamed: `build_placeholder` → `build_book_student`, `totals[:placeholders]` → `totals[:book_students]`, `ce_placeholders_count` → `ce_students_count`, provenance `book[:placeholders]` → `book[:students]`.
